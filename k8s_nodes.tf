resource "openstack_compute_instance_v2" "k8s_control_plane" {
  ...
  security_groups = [openstack_networking_secgroup_v2.gen3_ssh.name, openstack_networking_secgroup_v2.gen3_kubernetes.name, openstack_networking_secgroup_v2.gen3_k8s_control_plane.name]
  network {
...
resource "openstack_compute_instance_v2" "k8s_nodes" {
  ...
  key_pair        = openstack_compute_keypair_v2.gen3_ssh_key.name
  security_groups = [openstack_networking_secgroup_v2.gen3_kubernetes.name, openstack_networking_secgroup_v2.gen3_k8s_worker_node.name]
  network {
    name = openstack_networking_network_v2.gen3_network.name
  }
}
